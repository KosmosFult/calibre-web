...
def get_book_outline(book_id: int):
    try:
        outline = novel_service.get_book_outline(book_id=int(book_id))
        if not outline.get("chapters"):
            return json.dumps(
                {
                    "status": "empty",
                    "message": "Book outline not available. Build the narrative index first.",
                    "book_id": int(book_id),
                },
                ensure_ascii=False,
            )
...
def read_book_chapter(book_id: int, chapter_index: int = 0, max_words: int = 3000):
    try:
        chapter_payload = novel_service.get_chapter_chunks(
            book_id=int(book_id),
...
def read_book_segment(book_id: int, start_order_id: int, limit: int = 3):
    try:
        rows = novel_service.get_chunks_by_order_range(
            book_id=int(book_id),
...
def read_book_window(book_id: int, center_order_id: int, before: int = 1, after: int = 1):
    try:
        rows = novel_service.get_chunk_window(
            book_id=int(book_id),
...
def read_chapter_by_chunks(book_id: int, chapter_index: int, start_chunk_offset: int = 0, limit_chunks: int = 5):
    try:
        payload = novel_service.get_chapter_chunks(
            book_id=int(book_id),
            chapter_index=int(chapter_index),
...
@AgentTool(
    name="ask_novel",
    description=(
        "使用小说理解 Agent 的原文、层级记忆与事件图三通道回答一本书的问题。"
        "当用户提到剧情细节、角色关系、时间线、证据链、凶手推理等书内问题时优先调用。"
        "它返回带 [order_id=...] 引用、证据覆盖 trace 和不确定性的可验证答案草稿。"
        "主 Agent 可据此决定是否继续阅读原文、翻页核查，或用新的 probe 再次提问。"
...
def ask_novel(book_id: int, question: str, return_chunks: bool = False):
    try:
        result = novel_service.ask_book(book_id=int(book_id), question=question)
        ...
        condensed = {
            ...
            "evidence": (result.get("evidence") or [])[:10],
        ...
        log.exception("ask_novel failed")
        return json.dumps({"status": "error", "message": str(e)}, ensure_ascii=False)
...
def get_book_chunk_by_order(book_id: int, order_id: int):
    try:
        row = novel_service.get_chunk_by_order(book_id=int(book_id), order_id=int(order_id))
        if not row:
            return json.dumps(
                {
                    "status": "empty",
