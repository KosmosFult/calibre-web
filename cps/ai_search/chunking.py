...
class BookParser:
    ...
    def chunk_book(self, book_id: int, file_path: Optional[str] = None) -> List[str]:
        ...
        into chunks and persist them in the database. When contextual mode is enabled,
        each chunk will carry a `context_window` (neighboring text) and an `enriched_context`
        string generated by `ContextFuser`.
