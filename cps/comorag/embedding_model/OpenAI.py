    ...
    def __init__(self, global_config: Optional[BaseConfig] = None, embedding_model_name: Optional[str] = None) -> None:
        ...
        if self.global_config.azure_embedding_endpoint is None:
            self.provider_client = ProviderClient(
                provider=getattr(self.global_config, "embedding_provider", None),
        ...
        if len(texts) <= batch_size:
            ...
            for i in range(0, len(texts), batch_size):
                ...
                try:
                    ...
                    logger.error(f"Error encoding batch: {batch[:5]}... Retrying or skipping if max retries reached.", exc_info=True)
                    # Depending on desired behavior, you might want to re-raise, append None, or skip
                    # For now, let's assume a re-raise might be too disruptive if retries are handled by a decorator
                    # If no retry decorator handles this, this will propagate up.
                    pass # Or re-raise the exception if not handled elsewhere
