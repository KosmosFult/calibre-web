...
class CacheOpenAI(BaseLLM):
    ...
    def __init__(self, cache_dir, global_config, cache_filename: str = None,
        ...
        if self.global_config.azure_endpoint is None:
            self.provider_client = ProviderClient(
                provider=getattr(self.global_config, "llm_provider", None),
    ...
    def infer(
        ...
        **kwargs
        ...
        params = self._sanitize_params_for_compatible_endpoints(params)
        if self.openai_client is not None:
            # Azure path: keep original OpenAI-compatible call.
    ...
    def _sanitize_params_for_compatible_endpoints(self, params: dict) -> dict:
        """Remove fields unsupported by some OpenAI-compatible providers (e.g. Gemini)."""
        sanitized = deepcopy(params)
        model_name = str(sanitized.get("model", "")).lower()
        base_url = str(self.llm_base_url or "").lower()
        is_gemini_compat = ("generativelanguage.googleapis.com" in base_url) or ("gemini" in model_name)

        if is_gemini_compat:
            # Gemini OpenAI-compatible endpoint can reject these fields.
            for key in ("response_format", "seed", "n"):
