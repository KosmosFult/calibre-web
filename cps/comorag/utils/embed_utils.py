...
try:
    import torch
except ImportError:  # Optional for API-only mode
    torch = None
...
def retrieve_knn(query_ids: List[str], key_ids: List[str], query_vecs, key_vecs, k=2047, query_batch_size=1000,
    ...
    if torch is None:
        # Numpy fallback for environments without torch (e.g. API-only on macOS).
        query_vecs = np.asarray(query_vecs, dtype=np.float32)
        key_vecs = np.asarray(key_vecs, dtype=np.float32)
        query_norm = np.linalg.norm(query_vecs, axis=1, keepdims=True) + 1e-12
        key_norm = np.linalg.norm(key_vecs, axis=1, keepdims=True) + 1e-12
        query_vecs = query_vecs / query_norm
        key_vecs = key_vecs / key_norm

        similarities = np.dot(query_vecs, key_vecs.T)
        topk = min(k, len(key_ids))
        if topk <= 0:
            return {}

        results = {}
        topk_indices = np.argsort(similarities, axis=1)[:, -topk:][:, ::-1]
        topk_scores = np.take_along_axis(similarities, topk_indices, axis=1)
        for i, query_id in enumerate(query_ids):
            ids = [key_ids[idx] for idx in topk_indices[i].tolist()]
            results[query_id] = (ids, topk_scores[i].tolist())
        return results
