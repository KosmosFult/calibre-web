...
"""
    ...
    def load_and_validate_summaries(self, output_dir: str) -> bool:
        ...
        try:
            level_namespaces = self.summary_store.list_namespaces(prefix="level_")
            if not level_namespaces:
                logger.warning("No timeline embedding namespaces found in ai.db")
                return False

            levels = []
            for namespace in level_namespaces:
                if not namespace.startswith("level_"):
                    continue
                suffix = namespace.split("level_", 1)[1]
                if suffix.isdigit():
                    levels.append(int(suffix))

            if not levels:
                logger.warning("No valid level_* timeline namespaces found in ai.db")
                return False
            ...
            logger.info(f"Successfully loaded and validated timeline embeddings for {len(set(levels))} levels")
            return True
